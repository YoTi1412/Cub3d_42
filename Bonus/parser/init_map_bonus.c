...
/*   Updated: 2025/04/13 21:44:03 by wabolles         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	validate_color_format(char *color_string, t_map *map)
{
	...
	if (comma_count != 2)
		ft_error_map("Invalid color format: must have \
			exactly three numbers", map);
}
...
static unsigned int	init_colors(char *color_string, t_map *map)
{
	...
	if (color_string != NULL)
	{
		...
		if (array_size(rgb) != 3)
			ft_error_map_array("Wrong color format: must have exactly \
				three numbers", map, rgb);
		while (rgb[i])
		{
			if (error_color(rgb, map, i))
			{
				...
				if (colors[i] > 255)
					ft_error_map_array("Color values must be between \
						0 and 255", map, rgb);
			}
			i++;
		}
...
static void	init_map(t_map *map, t_dll *data)
{
	...
	ft_bzero(map->map, (map->height + 1) * sizeof(char *));
	i = 0;
...
t_map	*check_map(char *path)
{
	...
	data = read_map(path);
	if (!data)
		ft_error_map("Error reading map", NULL);
	map = ft_calloc(1, sizeof(t_map));
	if (!map)
	{
		free_dll_list(data);
		ft_error_map("Failed to allocate map struct", NULL);
	}
	map->data = data;
	...
	{
		free_map(map);
		ft_error_map("Map is empty/invalid", NULL);
	}
	validate_map(map);
	return (map);
}
